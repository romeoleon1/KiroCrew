...
One carve-out a fleet operator must plan around, on **Linux only**: the managed rung
binds the **gateway** and every process that shares its user namespace, and it does
**not** yet reach a process the gateway starts inside the namespace sandbox — every
script cron and every app backend. Inside that sandbox the kernel maps only the
real uid, so a root-owned file cannot be told from any other unmapped owner and the
trust check cannot accept it; rather than refuse to start, the child **skips the
managed rung**, runs one rung below the gateway (central document, then the local
file), and writes one latched audit row
(`security_policy_managed_skipped_in_sandbox`, `resources=managed<-userns`) that
your SIEM can alert on. A cron or app backend on a managed host is therefore bound by
the fleet's *central* document and its own local file, not by the managed file,
until the parent-authenticated carrier ([#9140](https://github.com/kirodotdev/KiroCrew/issues/9140))
hands the gateway's verified copy down a channel the child cannot forge. Two things
follow for a Linux fleet until then. The central document reaches those children,
so put the controls that must bind crons and app backends there — but read the
qualification above with the child in mind: the child verifies the central document
against `admission_policy.json`, the managed signature mandate does not apply inside
the sandbox (there is no managed rung to declare it), and the sandbox runs as the
local account. So a local account that can write or redirect its own admission trust
root can loosen what binds **its own sandboxed children** through either channel,
and no tier closes that until #9140. Treat a control that must hold against that
account on an unattended workload as **not yet enforceable on Linux**; the managed
file still binds the gateway and everything it does in-process, and the audit rows
above tell you which processes ran one rung below it. macOS is unaffected: its
children are not namespace-sandboxed.
