...
else creates cannot be misread that way, and
`test_the_managed_path_is_not_under_the_service_env_dir` pins the two apart.
...
row is written whichever document the mandate refused: the fetched one, or -- when the
source cannot be reached and the cache is the only document in hand -- the cached one.
An unsigned cache on an outage is that refusal, not an availability failure, so it
propagates marked rather than landing on the generic unavailable disposition. The
