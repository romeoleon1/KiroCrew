...
confident zero. A row that spells its command under any other key is a malformed
...
Until the seed lands, the fallback corpus is `scripts/deny_diff_fixture.json`;
the report always names which file answered and at which ref. The reason a row
belongs in the corpus is the reason a reviewer needs when the gate goes red, so a
row without one is not much use.
