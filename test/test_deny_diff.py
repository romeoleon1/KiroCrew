...
def _fake_tree(root: Path, denied: set[str] | dict[str, set[str]]) -> Path:
    ...
    *denied* is either a bare set (assigned to the rule-catalog tier, the common
    ...
    (package / "__init__.py").write_text(_FAKE_CLASSIFIER.format(tiers=tiers), encoding="utf-8")
    return root
...
def test_json_rendering_carries_the_rows_the_workflow_excuses(staged):
    """The workflow reads these rows to check a reviewer's comment names each one."""
    push = "git push origin feat/x"
