...
attempted, not that it failed. On the cron path five such cold-start stalls in
one bad hour marched ``consecutive_failures`` to the auto-pause threshold and
disabled a healthy ``silent`` job forever (issue #9522).

The fix carries the retry verdict on the type: ``AcpRequestTimeout.transient``
is ``True``, which ``acp_error_is_transient`` reads structurally (the same
channel ``AcpError.transient`` uses), so the cron callback's transient-retry arm
(``acp_error_is_transient(exc) and not _prompt_dispatched``) fires instead of
recording a failure. Deciding by type rather than by a ``"timed out"`` string in
``_TRANSIENT_MARKERS`` is the point: a reword of the message cannot silently flip
retryability (the root pattern #7395 names).
"""
...
class TestHandshakeTimeoutIsStructurallyTransient:
    def test_request_timeout_is_transient(self) -> None:
        # The real message the runtime raises (runtime.py:_send_and_await).
    ...
    def test_verdict_comes_from_type_not_wording(self) -> None:
        ...
        # This is the #7395 guard: reword the message and retryability holds.
    ...
    def test_enriched_replacement_stays_transient(self) -> None:
        # runtime._session_start_stalled raises a NEW AcpRequestTimeout wrapping
        # the original plus MCP progress; the class attribute must survive.
    ...
    def test_bare_runtime_error_is_not_transient(self) -> None:
        ...
        # AcpRuntimeError still fails fast — only the timeout subclass is retried.
    ...
    def test_prompt_timeout_unaffected(self) -> None:
        ...
        # error / message), unchanged by this fix — default is unclassified.
