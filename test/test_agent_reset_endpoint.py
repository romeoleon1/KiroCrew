...
rebind-then-delete (GPT round-13): the origin must exist and the crew must
...
async def test_reset_removes_copy_whose_file_stem_differs_from_name(tmp_path):
    """GPT round-14 (F2): the delete must resolve the copy's ACTUAL file, not
...
async def test_reset_keeps_copy_bound_by_file_stem(tmp_path):
    """GPT round-31: a stem binding (stem != declared name) resolves the same
