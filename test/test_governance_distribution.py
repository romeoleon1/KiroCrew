...
class TestAManagedDeclarationMandatesASignedCentralDocument:
    ...
    def test_a_signed_document_is_adopted_under_a_managed_declaration(self, transport, monkeypatch):
        ...
        source = transport(_static_fetcher(pd.FetchedPolicy(body=body)))
        monkeypatch.setattr(
            governance, "_policy_trust_settings", lambda *_a: (False, {"corp": "k"})
        )
    ...
    # -- a migration refused under a managed declaration names the managed document --
    #
    # ``_env_pins_the_source`` is False under a managed declaration, so a moved
    # ``distribution.source`` is refused there too. The refusal's remedy has to name a
    # lever the operator can pull: under a managed pin the env knobs and the placed
    # policy are ignored (``resolve_distribution`` refuses them), so the address can
    # only move by editing the managed document. Directing the operator at
    # ``KIROCREW_POLICY_URL`` would send them to a channel that does nothing.
    #
    # The env var is left UNSET here on purpose: a set ``KIROCREW_POLICY_URL`` makes
    # the standalone compose that ``parse_policy`` triggers on the way in fetch from
    # it and spend the per-process fetch slot, which would throttle the fetch under
    # test. The managed branch ignores the variable either way.

    @staticmethod
    def _signed_body(marker: str, **extra: object) -> bytes:
        return json.dumps(_sign(_doc(marker, identity={"issuer": "corp"}, **extra), "k")).encode()

    @staticmethod
    def _trust_corp(monkeypatch) -> None:
        monkeypatch.setattr(
            governance, "_policy_trust_settings", lambda *_a: (False, {"corp": "k"})
        )

    def test_a_moved_source_at_boot_names_the_managed_document_not_the_env(
        self, transport, monkeypatch
    ):
        self._trust_corp(monkeypatch)
        monkeypatch.delenv(pd.POLICY_URL_ENV, raising=False)
        moved = self._signed_body(
            "fleet", distribution={"source": "https://elsewhere.corp.example/p.json"}
        )
        source = transport(_static_fetcher(pd.FetchedPolicy(body=moved)))
        with pytest.raises(PlatformCompositionError) as caught:
            pd.load_distributed_policy(self._managed(source))
        message = str(caught.value)
        assert "moves distribution.source" in message
        assert "managed document" in message
        assert pd.POLICY_URL_ENV not in message, "the env knob is inert under a managed pin"

    def test_a_moved_source_on_refresh_names_the_managed_document_not_the_env(
        self, transport, install_ceiling, monkeypatch
    ):
        from dataclasses import replace
        ...
        self._trust_corp(monkeypatch)
        monkeypatch.delenv(pd.POLICY_URL_ENV, raising=False)
        source = transport(_static_fetcher(pd.FetchedPolicy(body=self._signed_body("v1"))))
        # ``managed`` on the pins is stamped by the composer (``_managed_ceiling``), not
        # by ``parse_policy``; install the pins the way the composer would leave them.
        install_ceiling(
            replace(
                governance.parse_policy(_doc("running", distribution={"source": source})),
                distribution=self._managed(source),
            )
        )
        moved = self._signed_body(
            "v2", distribution={"source": "https://elsewhere.corp.example/p.json"}
        )
        transport(_static_fetcher(pd.FetchedPolicy(body=moved, etag="v2")))

        outcome = pd.refresh_now()
        assert outcome.status == pd.REFRESH_REJECTED
        assert "managed document" in outcome.detail
        assert pd.POLICY_URL_ENV not in outcome.detail

    # -- an unsigned cache under the managed mandate is a refusal, not an outage --
    #
    # Managed declaration with ``on_unavailable: degrade``, a cache the mandate refuses,
    # and the endpoint down: the cache is the only document in hand and it fails the
    # mandate. That refusal has to reach ``load_security_policy`` MARKED, so the
    # ``security_policy_managed_delegation_unverified`` row is written and central is
    # off for the reason the row names -- not swallowed into a generic "unavailable"
    # that ``degrade`` turns into a silent None.

    def test_an_unsigned_cache_on_outage_surfaces_the_managed_refusal(self, transport, monkeypatch):
        monkeypatch.setattr(governance, "_policy_signature_required", lambda: False)
        pd.write_cache(_body("planted"), source=_TEST_SOURCE, etag="v1")
        source = transport(_failing_fetcher(TimeoutError("the endpoint is down")))
        dist = pd.PolicyDistribution(
            source=source, managed=True, on_unavailable=governance.UNAVAILABLE_DEGRADE
        )
        with pytest.raises(PlatformCompositionError) as caught:
            pd.load_distributed_policy(dist)
        assert governance._managed_delegation_unverified(
            caught.value
        ), "the loader must see the mandate refusal, not a bare outage"
        assert "the endpoint is down" in str(caught.value) or "could not fetch" in str(caught.value)

    def test_a_refused_document_with_an_unsigned_cache_keeps_its_unmarked_abort(
        self, transport, monkeypatch
    ):
        """The control: the source was REACHED and its document refused for a reason
        other than the mandate. That raise stays a boot abort under any disposition,
        so it must not pick up the cache's mandate marker on the way out."""
        monkeypatch.setattr(governance, "_policy_signature_required", lambda: False)
        pd.write_cache(_body("planted"), source=_TEST_SOURCE, etag="v1")
    ...
    def test_an_unsigned_cache_on_outage_is_still_served_off_the_managed_tier(
        self, transport, monkeypatch
    ):
        """The other control: with no managed mandate an unsigned cache is a valid
        last-known-good, and an outage serves it."""
        monkeypatch.setattr(governance, "_policy_signature_required", lambda: False)
        pd.write_cache(_body("lkg"), source=_TEST_SOURCE, etag="v1")
        source = transport(_failing_fetcher(TimeoutError("the endpoint is down")))
        ceiling = pd.load_distributed_policy(pd.PolicyDistribution(source=source))
        assert ceiling is not None and ceiling.identity_issuer == "lkg"

    def test_a_moved_source_on_an_unmanaged_host_still_names_the_env(
        self, transport, install_ceiling, monkeypatch
    ):
        """The control: the operator's own host is bootstrapped by the env var or the
        placed policy, and those are the levers the refusal must name."""
        monkeypatch.delenv(pd.POLICY_URL_ENV, raising=False)
        source = transport(_static_fetcher(pd.FetchedPolicy(body=_body("v1"))))
        install_ceiling(governance.parse_policy(_doc("running", distribution={"source": source})))
        moved = _body("v2", distribution={"source": "https://elsewhere.corp.example/p.json"})
        ...
        outcome = pd.refresh_now()
        assert outcome.status == pd.REFRESH_REJECTED
        assert pd.POLICY_URL_ENV in outcome.detail
        assert "managed document" not in outcome.detail
...
class TestAKeylessManagedDelegationLeavesCentralOffRatherThanAbortingBoot:
    ...
    def _managed_delegating(
        ...
        monkeypatch.setattr(governance, "_policy_home_path", lambda: tmp_path / "absent.json")
        monkeypatch.setattr(governance, "_policy_signature_required", lambda: False)
        monkeypatch.setattr(governance, "_policy_trust_settings", lambda *_a: (False, {}))
        monkeypatch.setattr(governance, "_policy_asymmetric_settings", lambda *_a: {})
    ...
    def test_a_non_managed_declaration_with_the_opt_in_still_aborts(
        ...
        monkeypatch.setattr(governance, "_policy_home_path", lambda: home)
        monkeypatch.setattr(governance, "_policy_signature_required", lambda: True)
        monkeypatch.setattr(governance, "_policy_trust_settings", lambda *_a: (True, {}))
        with pytest.raises(PlatformCompositionError):
            governance.load_security_policy()
