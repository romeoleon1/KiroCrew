...
def test_github_monitor_resolution_requires_the_same_protected_executable(monkeypatch):
    """GitHub must not be the one monitor allowed to execute an agent-writable CLI."""
    validations: list[tuple[str, bool]] = []
    monkeypatch.delenv("KIROCREW_GH_BIN", raising=False)
    monkeypatch.setattr(
        provider_cli_module,
        "provider_executable_candidates",
        lambda _executable: ("/provider/gh",),
    )
    ...
    assert provider_cli_module.resolve_provider_cli("gh") == "/provider/gh"
    assert validations == [("/provider/gh", True)]
