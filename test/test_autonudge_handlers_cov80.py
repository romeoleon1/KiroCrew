...
async def test_legacy_list_omits_cycle_accounting_only_for_structured_rows(
    ...
    """The unmaintained half of the withheld set, asserted on both sides.

    A structured monitor's ``max_cycles``/``cycle_count``/``last_fire_ts`` are
    stuck at the zero every arming call writes, and nothing on its tick path
    advances them, so the legacy reading ("no cycle cap", "has not run yet") would
    be false about a record bounded by ``monitor.budgets``. A plain loop keeps all
    three, because for it they are real.
    """
    legacy = _loop("lp-1")
    ...
    structured = _monitor_loop("mon-1", "chat-2-222")
    structured.monitor.probe_count = 12  # type: ignore[union-attr]
    _svc(monkeypatch, _FakeSvc([legacy, structured]))
    ...
    for withheld in ("max_cycles", "cycle_count", "last_fire_ts"):
        assert withheld not in rows["mon-1"]
    # Nothing replaces them: "how far in" is UNANSWERED on this route, which is
    # recorded as a real gap rather than filled with a plausible zero.
...
async def test_legacy_get_returns_a_structured_monitor(
    ...
    assert "monitor" not in payload["loop"]
    assert "max_cycles" not in payload["loop"]
    assert "must-not-escape" not in json.dumps(payload)
...
async def test_both_legacy_reads_return_a_monitor_armed_through_the_create_route(
    ...
    for row in (listed["loops"][0], per_slot["loop"]):
        ...
        for withheld in ("monitor", "message", "max_cycles", "cycle_count"):
