...
async def test_fork_rollback_keeps_a_foreign_bound_copy(tmp_path):
    ...
    only the failed fork's lineage (GPT round-50). The bind lands DURING the
    window here: a pre-existing binding is already reserved at naming time
    (round-51), so it can no longer stage this race."""
    agents_dir = tmp_path / "agents"
    agents_dir.mkdir()
    _write_template(agents_dir, "mytemplate")
    _seed_config("design-crew", "mytemplate")
    ...
    def _bind_then_fail(*args, **kwargs):
        ...
        # lineage is recorded — then the sidecar write fails.
        cfg = KiroCrewConfig()
        cfg.agents = {
            "design-crew": KiroCrewAgentConfig(kiro_agent="mytemplate"),
            "other-crew": KiroCrewAgentConfig(kiro_agent="design-crew"),
        }
...
async def test_fork_prunes_stale_sidecar_state_on_reused_name(tmp_path):
    """GPT round-19: a destination NAME that lived before (delete whose sidecar
...
async def test_fork_clears_a_stale_refresh_failure(tmp_path, monkeypatch):
    """GPT round-44: a refresh interleaving between the lineage record and the
