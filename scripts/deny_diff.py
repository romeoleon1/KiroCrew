...
def classify(checkout: Path, commands: list[str], *, home: Path) -> list[Verdict]:
    ...
    if not commands:
        return []
    request = {"commands": commands, "expect_root": str((checkout / "src").resolve())}
    proc = subprocess.run(
...
def _worker_main() -> int:
    ...
    # would refuse in production. A tier this ref does not carry is an error rather
    # than a skip: a missing check is coverage silently lost, which is the failure
...
def differential(
    ...
    *resolver* defaults to the module-level :func:`resolve_checkout` read at CALL
    ...
    base_verdicts = classify(base_tree, commands, home=base_home)
    head_verdicts = classify(head_tree, commands, home=head_home)
