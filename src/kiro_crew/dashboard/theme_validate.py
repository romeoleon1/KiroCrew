...
# The loader is a purely visual chat-footer indicator. It has no legitimate need
# for audio/video, and the theme audio path is consent- and mute-gated in the
# dashboard — so a loader iframe must not be able to play sound out of band.
# Same as the overlay CSP but with media-src 'none'.
_THEME_LOADER_CSP = (
    "default-src 'none'; script-src 'unsafe-inline'; style-src 'unsafe-inline'; "
    "img-src data:; media-src 'none'; connect-src 'none'; form-action 'none'; "
    "sandbox allow-scripts"
)
