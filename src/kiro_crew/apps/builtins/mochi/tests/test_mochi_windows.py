...
def test_manifest_declares_every_platform_the_app_runs_on():
    """Mochi's backend is in-process gateway routes plus hooks -- no exec backend,
    no subprocess anywhere in the app -- so nothing in it is POSIX-bound.

    Pinned because omitting the block falls back to the implicit
    ``["macos", "linux"]`` default, which silently drops Windows and is the only
    thing that ever blocked the app there.
    """
